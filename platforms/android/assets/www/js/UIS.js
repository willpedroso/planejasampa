...
var UIS = {
  ...
  configListeners: function() {
    ...
    this.select_ListaSubprefeituras.bind("change", (function(event) {
      ...
      $('#listaSubprefeituras_metas option[value="' + $("#selectListaSubprefeituras option:selected").val() + '"]').attr({
        selected: "selected"
      });
    ...
    this.select_ListaSubprefeiturasMetas.bind("change", (function(event) {
      ...
      $('#selectListaSubprefeituras option[value="' + $("#listaSubprefeituras_metas option:selected").val() + '"]').attr({
        selected: "selected"
      });
  ...
  showTelaAlert: function(msgErro, msgBotao, func) {
    // Mostra a tela de aguarde
    //this.div_Aguarde.removeClass('dspl-nn');
    var msg = "";
...
//      "<button class='bt_alert' onclick=\"UIS.hideTelaAguarde('#div_Aguarde');\">" + msgBotao + "</button>" +
      "<button class='bt_alert' onclick=\"UIS.hideTelaAguarde('#div_Aguarde');" + func + "\">" + msgBotao + "</button>" +
      "</div>"
      + "</div>";
  ...
  showDetalhesMetas: function(dados, projetos) {
    ...
    for (var i = 0; i < dados.rows.length; i++) {
      nodes +=
        "<li class=''>" +
        "<div class='col-50-l  marg-t-16 metaSelecionada'>" +
        ...
        "</span>" +
        "</div>" +
        "<div class='valor_perc font-4-em  marg-t-16 cor_cz3e font-b lettr-spacing-tit-pct'>" +
        //"100%" +
      ...
      for (var j = 0; j < projetos.rows.length; j++) {
        listaTiposProjetos += "<li class='cor_cz hspacer-line' idProjeto='" +
    ...
    $('#recebeSelectMeta').empty();
    $('#recebeSelectMeta').append(UIS.metaSelecionada);
  ...
  fillDivGoalsPrefecture: function(dados, listaStatus, listaObjetivos, idStatus, idObjective) {
    ...
    var alturaSubpref = document.getElementById('bt_Subprefeituras').clientHeight;
    var alturaFooter = document.getElementById('div_footer').clientHeight;
    var alturaSubtrair = alturaHeader + alturaSubpref + alturaFooter;
    var TamTela = window.innerHeight - alturaSubtrair;
    ...
    linhasOrdenadas.sort(function(a, b) {
      return b - a
    });
  ...
  fillDivStatusGoals: function(dados) {
    ...
    for (var i = 0; i < dados.rows.length; i++) {
      linhas[i] = dados.rows.item(i).QTD;
    }
    var percents = [];
    var alturas = [];
    var total = 0;
    ...
    for (var i = 0; i < linhas.length; i++) {
      ...
      nodes += "<div idRegistro='' class='metas_andamento divide_meta'" + "style='height:" + alturas[i] + "px; box-sizing: border-box;'>" +
        "<div idRegistro='" + dados.rows.item(i).STATUS_META + "' countRegistro='" + linhas[i] + "' class='descricao_andamento  gray_4'>" +
        this.txtStatusMetas[dados.rows.item(i).STATUS_META] +
        "</div>" +
        "<div idRegistro='" + dados.rows.item(i).STATUS_META + "' countRegistro='" + linhas[i] + "' class='valor_andamento cor_valor_meta " + fonteCorreto + "'>" +
        linhas[i] +
        "</div>" +
        "</div>";
    }
